...
beforeEach(async () => {
    ...
    const newUser = new User({ ...user, passwordHash: await bcrypt.hash(user.password, 10) });
    await newUser.save();
...
describe(`${dataName} tests`, () => {
    describe(`when there is initially some ${dataName}s saved`, () => {
        ...
        test(`all ${dataName}s are returned`, async () => {
            const res = await api.get(baseRoute).set({ Authorization: `Bearer ${user.token}` });
    ...
    describe(`adding a new ${dataName}`, () => {
        ...
        test('fails with status code 400 if data is invalid', async () => {
            ...
            const response = await api.get(baseRoute).set({ Authorization: `Bearer ${user.token}` });
    ...
    describe(`updating a ${dataName}`, () => {
        test(`succeeds with status code 200 if ${dataName} exists and data is valid`, async () => {
            ...
            const dataUpdated = await api.get(`${baseRoute}/${dataToUpdate.id}`).set({ Authorization: `Bearer ${user.token}` });
    ...
    describe(`specific ${dataName}s tests`, () => {
        test('if likes property is missing, defaults to 0', async () => {
            ...
            const res = await api.post(baseRoute).send(newBlog).set({ Authorization: `Bearer ${user.token}` });
